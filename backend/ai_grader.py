...
def validate(parsed: dict):
    ...
    for criterion in REQUIRED_CRITERIA:
        ...
        if criterion not in scores:
            raise ValueError(f"{criterion}: score 누락")

        score = normalize_score(scores.get(criterion))
        parsed_scores[criterion] = score

        r = rationales.get(criterion)
        if not isinstance(r, list):
            raise ValueError(f"{criterion}: rationales 형식 오류")
...
아래 채점기준표의 각 평가영역별 헥심 평가요소를 고려하여 10점 만점으로 평가합니다. 

[채점기준표]

평가영역 1. 수과학적 지식(Scientific Knowledge)
[핵심 평가요소]
...
평가영역 2. 비판적 사고력(Critical Thinking)
[핵심 평가요소]
...
각 평가영역에 대해 1~10점 사이의 정수로 평가합니다.
